...
$t$, with $t_\mathcal{L}(\alpha) = \texttt{let $\emptyset$ in $f
  ~\widehat{xs}$}$ and $t_\mathcal{L}(\beta) = \texttt{let $\emptyset$
  in $f [a, as]$}$, with $\alpha$ being an ancestor of $\beta$, we are
...
for any infinite sequence $\widehat{t_1}, \widehat{t_2}, ...$ of
terms, there is guaranteed to be $\widehat{t_i} <| \widehat{t_j}$ for
$i < j$. The homeomorphic embedding relation on C-terms is the
...
\begin{algorithmic}
  ...
  \WHILE{$t$ is not closed}
    ...
    \ELSE
      ...
      \IF{$t_\mathcal{L}(\alpha) \gen t_\mathcal(\beta)$}
...
% \texttt{Add} program. At the root node it is determined whether the
% value stored in $X_{in}$ is an atom (left) or a cons (right). When
% branching left, we bind $X_{in}$ to a fresh atom
% meta-variable. Depending of the outcome of the equality test we return
% an error or the $y$-value. On the right branch we haveIf we instead branch right, we 
