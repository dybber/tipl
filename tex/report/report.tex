...
possible value. They should not be confused with \textit{program
  variables}, as these are used in the specification of a program
only, will never represent more than a single value at a time.
...
A concrete value is said to be instance of an expression containing
...
input such that the path is taken. The universal resolving algorithm
...
\in E$, for some $E$. The $\Gamma$ map which will be explained later,
is essentially a static specification of which variables are in scope
in a given state.
...
%\subsubsection{Backwards interpretation}
%When backwards interpreting the add program (see Figure
%\ref{fig:addtree-rev}), we begin in the node labeled ``$X_{out}$'', 
...
\end{tikzpicture}
\caption{Reversed closed process tree for checking if a list consists
  of inequal pairs of atoms.}
\label{fig:neq-rev}
\end{figure*}
