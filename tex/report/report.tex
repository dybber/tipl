...
by after a generalization step (the edge is implicit in our
...
\node[vtx] (N2)  {\texttt{Add x y}} [->]
            child [sibling distance=6cm, level distance=1cm] {
                node[vtx] (N3)  {\texttt{if ...}}
                    ...
                    child [sibling distance=3cm, level distance=1.5cm]{
                        node[leaf] (N5)  {\texttt{y}}
                        edge from parent {
                          node[right] {$Xa_1 \mapsto \texttt{'Z}$}
                        }
                    }
                    edge from parent {
                      node[left] {$x \mapsto Xa_1\quad$}
                    }
            }
            child [sibling distance=7cm, level distance=1cm]{
                node[vtx] (N6)  {
                  $\begin{array}{l}
                    \texttt{let x = $Xe_2$, y = ['S, y]} \\
                    \texttt{in Add x y}
                  \end{array}$
                }
